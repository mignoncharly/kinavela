...
begin
  ...
  update public.notification_outbox set
    ...
    error_code=case when attempts>=5
      then 'delivery_retries_exhausted' else error_code end,
    claimed_at=null,
  ...
  select claimed.id,claimed.recipient_profile_id,claimed.channel,
    ...
    case claimed.channel
      when 'email' then
        kinavela_private.feature_enabled(
          'notifications_email',claimed.recipient_profile_id
        )
        and coalesce(preferences.email_enabled,false)
        and exists (
          select 1 from public.consents consent
          where consent.profile_id=claimed.recipient_profile_id
      ...
      when 'push' then
        ...
        and coalesce(preferences.push_enabled,false)
        and exists (
          select 1 from public.notification_push_subscriptions subscription
          where subscription.profile_id=claimed.recipient_profile_id
        )
      else true end,
    claimed.payload
  from claimed
  ...
  left join public.notification_preferences preferences
    on preferences.profile_id=claimed.recipient_profile_id;
end;
$$;
...
values('202608130022_notification_claim_consent')
