...
| Messaging and safety  | conversations, messages, reports, Village moderation                           | Messages, reports, moderation metadata                                              | Communication and safety                      |
...
| Operations            | audit events, request rate limits, product events, geocoding cache             | Security metadata, hashed identifiers, opt-in metrics, city-search cache            | Security and operations                       |
...
The database retention migration and privacy cron cover exports, story requests/media, notification queues/events, event reminders, product metrics, geocoding cache/rate-limit hashes, AI job data if ever enabled, and short-lived operational records. Active account content remains until the account deletion workflow; private child/story media is removed first.
