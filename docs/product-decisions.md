...
- Realtime uses filtered Postgres Changes for the initial pilot. The client ignores row payload content and refreshes the authorized server projection. Move to private Broadcast if measured concurrent subscription volume outgrows Postgres Changes.
